...
public:
    ...
    const_iterator begin() const {
        return static_cast<const SubTrie *>(mRoot.get())->begin();
    }
    ...
    std::optional<SubTrie> getSubTrie(key_type subKey) {
        return mRoot->getSubTrie(subKey);
    }

protected:
    std::shared_ptr<SubTrie> mRoot;
...
public:
    ...
    protected:
        ...
        std::shared_ptr<SubTrie> thisShared() const {
        ...
        size_t eraseInternal(iterator position) {
            ...
            auto subTrie = position.mCurrentSubTrie.lock();
            size_t count = 0;
        ...
        const Trie *mTrie = nullptr;

        std::map<char, std::shared_ptr<SubTrie>> mChildren;

        std::weak_ptr<SubTrie> mParent;
    ...
    protected:
        explicit TrieIterator(std::weak_ptr<SubTrie> root, bool end = false)
                : mRoot(root) {
            if (!end) {
        ...
        explicit TrieIterator(std::weak_ptr<SubTrie> root, std::weak_ptr<SubTrie> current)
        ...
        std::weak_ptr<SubTrie> mRoot;

        std::weak_ptr<SubTrie> mCurrentSubTrie;
